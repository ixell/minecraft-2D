...
class Player(pg.sprite.Sprite):
    ...
    def sprites_move_to(self, pos):
        if self.direction in (0, 2):
            ...
            self.lhand.move_to((pos[0], pos[1] + self.head.rect.h))
            self.body.move_to((pos[0] + self.lhand.rect.w, pos[1] + self.head.rect.h))
            self.rhand.move_to((pos[0] + self.lhand.rect.w + self.body.rect.w,
                pos[1] + self.head.rect.h))
            self.lleg.move_to((pos[0] + self.lhand.rect.w, 
            pos[1] + self.head.rect.h + self.body.rect.h))
    ...
    def sprites_rotate(self):
        ...
        for n, i in [('rleg', self.rleg), ('lleg', self.lleg), ('rhand', self.rhand), ('lhand', self.lhand)]:
            # _ = i.rerect(i.last_rect.right) if self.rots[n][0] < 0 else i.rerect(i.last_rect.left)
            i.rerect()
        self.rots['rleg'][0] += self.rots['rleg'][1]
        self.rots['lleg'][0] += self.rots['lleg'][1]
        self.rots['rhand'][0] += self.rots['rhand'][1]
        self.rots['lhand'][0] += self.rots['lhand'][1]
        for k in self.rots:
            if abs(self.rots[k][0]) >= 90:
    ...
    def resprite(self, num):
        ...
        self.sprites.add(self.head, self.rleg, self.lleg,
                         self.rhand, self.lhand, self.body)
